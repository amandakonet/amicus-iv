...
mod15 <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + lcdisdirfem + pfemam + 
                relauf_overturn_roe + relauf_overturn_roe:justicemq,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
...
```{r}
interact_plot(mod15, pred = relauf_overturn_roe, modx = justicemq,
              #modx.values = c(-3.5, -1, 0, 1, 3.5),
              # random
              #modx.values = c(-1.65, -.55, -.08, .35, 1.5),
              # quantiles
...
```{r, eval=F}
# interaction levels
ef <- effect(term="justicemq:relauf_overturn_roe", mod = mod15)
#ef1<-effect(term="pfemau*justicemq",xlevels=list(pfemau=c(0,.5,1)),mod=res0)  #used this in paper

# as dataframe
ef_df <-as.data.frame(ef) %>% mutate(relauf_overturn_roe = as.factor(relauf_overturn_roe))

#By Color 
ggplot(ef_df, aes(x = justicemq, y= fit, group = relauf_overturn_roe))+
  geom_point()+
  geom_line(size = 1, aes(linetype = relauf_overturn_roe))+
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=relauf_overturn_roe),alpha=0.5) + 
...
mod22 <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + lcdisdirfem + pfemam + 
                relauf_uncertainty + relauf_uncertainty:justicemq,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
...
```{r}
interact_plot(mod22, pred = relauf_uncertainty, modx = justicemq,
              #modx.values = c(-3.5, -1, 0, 1, 3.5),
              # random
              #modx.values = c(-1.65, -.55, -.08, .35, 1.5),
              # quantiles
...
```{r, eval=F}
# interaction levels
ef <- effect(term="justicemq:relauf_uncertainty", mod = mod22)
#ef1<-effect(term="pfemau*justicemq",xlevels=list(pfemau=c(0,.5,1)),mod=res0)  #used this in paper

# as dataframe
ef_df <-as.data.frame(ef) %>% mutate(relauf_uncertainty = as.factor(relauf_uncertainty))

#By Color 
ggplot(ef_df, aes(x = justicemq, y= fit, group = relauf_uncertainty))+
  geom_point()+
  geom_line(size = 1, aes(linetype = relauf_uncertainty))+
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=relauf_uncertainty),alpha=0.5) + 
...
mod24 <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + lcdisdirfem + pfemam + 
                relauf_untruthful + relauf_untruthful:justicemq,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
...
```{r}
interact_plot(mod24, pred = relauf_untruthful, modx = justicemq,
              #modx.values = c(-3.5, -1, 0, 1, 3.5),
              # random
              #modx.values = c(-1.65, -.55, -.08, .35, 1.5),
              # quantiles
...
```{r, eval=F}
# interaction levels
ef <- effect(term="justicemq:relauf_untruthful", mod = mod24)
#ef1<-effect(term="pfemau*justicemq",xlevels=list(pfemau=c(0,.5,1)),mod=res0)  #used this in paper

# as dataframe
ef_df <-as.data.frame(ef) %>% mutate(relauf_untruthful = as.factor(relauf_untruthful))

#By Color 
ggplot(ef_df, aes(x = justicemq, y= fit, group = relauf_untruthful))+
  geom_point()+
  geom_line(size = 1, aes(linetype = relauf_untruthful))+
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=relauf_untruthful),alpha=0.5) + 
...
mod27 <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + lcdisdirfem + pfemam + 
                relauf_womens_rights + relauf_womens_rights:justicemq,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
...
```{r}
interact_plot(mod27, pred = relauf_womens_rights, modx = justicemq,
              #modx.values = c(-3.5, -1, 0, 1, 3.5),
              # random
              #modx.values = c(-1.65, -.55, -.08, .35, 1.5),
              # quantiles
...
```{r, eval=F}
# interaction levels
ef <- effect(term="justicemq:relauf_womens_rights", mod = mod27)
#ef1<-effect(term="pfemau*justicemq",xlevels=list(pfemau=c(0,.5,1)),mod=res0)  #used this in paper

# as dataframe
ef_df <-as.data.frame(ef) %>% mutate(relauf_womens_rights = as.factor(relauf_womens_rights))

#By Color 
ggplot(ef_df, aes(x = justicemq, y= fit, group = relauf_womens_rights))+
  geom_point()+
  geom_line(size = 1, aes(linetype = relauf_womens_rights))+
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=relauf_womens_rights),alpha=0.5) + 
