...
mod15 <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + lcdisdirfem + pfemam + 
                medauf_overturn_roe + medauf_overturn_roe:justicemq,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
...
```{r}
interact_plot(mod15, pred = medauf_overturn_roe, modx = justicemq,
              #modx.values = c(-3.5, -1, 0, 1, 3.5),
              # random
              #modx.values = c(-1.65, -.55, -.08, .35, 1.5),
              # quantiles
...
```{r, eval=F}
# interaction levels
ef <- effect(term="justicemq:medauf_overturn_roe", mod = mod15)
#ef1<-effect(term="pfemau*justicemq",xlevels=list(pfemau=c(0,.5,1)),mod=res0)  #used this in paper

# as dataframe
ef_df <-as.data.frame(ef) %>% mutate(medauf_overturn_roe = as.factor(medauf_overturn_roe))

#By Color 
ggplot(ef_df, aes(x = justicemq, y= fit, group = medauf_overturn_roe))+
  geom_point()+
  geom_line(size = 1, aes(linetype = medauf_overturn_roe))+
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=medauf_overturn_roe),alpha=0.5) + 
...
mod22 <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + lcdisdirfem + pfemam + 
                medauf_uncertainty + medauf_uncertainty:justicemq,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
...
```{r}
interact_plot(mod22, pred = medauf_uncertainty, modx = justicemq,
              #modx.values = c(-3.5, -1, 0, 1, 3.5),
              # random
              #modx.values = c(-1.65, -.55, -.08, .35, 1.5),
              # quantiles
...
```{r, eval=F}
# interaction levels
ef <- effect(term="justicemq:medauf_uncertainty", mod = mod22)
#ef1<-effect(term="pfemau*justicemq",xlevels=list(pfemau=c(0,.5,1)),mod=res0)  #used this in paper

# as dataframe
ef_df <-as.data.frame(ef) %>% mutate(medauf_uncertainty = as.factor(medauf_uncertainty))

#By Color 
ggplot(ef_df, aes(x = justicemq, y= fit, group = medauf_uncertainty))+
  geom_point()+
  geom_line(size = 1, aes(linetype = medauf_uncertainty))+
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=medauf_uncertainty),alpha=0.5) + 
...
mod24 <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + lcdisdirfem + pfemam + 
                medauf_untruthful + medauf_untruthful:justicemq,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
...
```{r}
interact_plot(mod24, pred = medauf_untruthful, modx = justicemq,
              #modx.values = c(-3.5, -1, 0, 1, 3.5),
              # random
              #modx.values = c(-1.65, -.55, -.08, .35, 1.5),
              # quantiles
...
```{r, eval=F}
# interaction levels
ef <- effect(term="justicemq:medauf_untruthful", mod = mod24)
#ef1<-effect(term="pfemau*justicemq",xlevels=list(pfemau=c(0,.5,1)),mod=res0)  #used this in paper

# as dataframe
ef_df <-as.data.frame(ef) %>% mutate(medauf_untruthful = as.factor(medauf_untruthful))

#By Color 
ggplot(ef_df, aes(x = justicemq, y= fit, group = medauf_untruthful))+
  geom_point()+
  geom_line(size = 1, aes(linetype = medauf_untruthful))+
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=medauf_untruthful),alpha=0.5) + 
...
mod27 <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + lcdisdirfem + pfemam + 
                medauf_womens_rights + medauf_womens_rights:justicemq,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
...
```{r}
interact_plot(mod27, pred = medauf_womens_rights, modx = justicemq,
              #modx.values = c(-3.5, -1, 0, 1, 3.5),
              # random
              #modx.values = c(-1.65, -.55, -.08, .35, 1.5),
              # quantiles
...
```{r, eval=F}
# interaction levels
ef <- effect(term="justicemq:medauf_womens_rights", mod = mod27)
#ef1<-effect(term="pfemau*justicemq",xlevels=list(pfemau=c(0,.5,1)),mod=res0)  #used this in paper

# as dataframe
ef_df <-as.data.frame(ef) %>% mutate(medauf_womens_rights = as.factor(medauf_womens_rights))

#By Color 
ggplot(ef_df, aes(x = justicemq, y= fit, group = medauf_womens_rights))+
  geom_point()+
  geom_line(size = 1, aes(linetype = medauf_womens_rights))+
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se, fill=medauf_womens_rights),alpha=0.5) + 
