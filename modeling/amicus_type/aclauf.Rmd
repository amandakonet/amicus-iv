---
title: "Aclauf Amicus Brief Models"
author: "Amanda Konet"
date: "`r Sys.Date()`"
output: pdf_document
---
...
data <- box_read(amicus_only) %>% 
  ...
  mutate(across(c(aclauf_abortion_different:aclauf_womens_rights), ~replace_na(.x, 0)))
```
...
```{r}
data <- data %>%
  select(case:cirsplit, aclauf_abortion_different:aclauf_womens_rights) %>% 
  mutate(across(c(where(is.numeric), -justicevot, -id, -cirsplit),
                ~(scale(., center=T, scale=T))))
                #~(scale(., center=T, scale=T) %>% as.vector)))

total_cols <- data %>% select(contains('aclauf')) %>% colnames()
data <- janitor::remove_empty(data, which = "cols")
remaining_cols <- data %>% select(contains('aclauf')) %>% colnames()
...
mod6b <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + nonftyp2 + lcdisdirfem +
                aclauf_compulsory_motherhood_ii,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
