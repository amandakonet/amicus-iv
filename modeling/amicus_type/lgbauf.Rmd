---
title: "Lgbauf Amicus Brief Models"
author: "Amanda Konet"
date: "`r Sys.Date()`"
output: pdf_document
---
...
data <- box_read(full_cases) %>% 
  ...
  mutate(across(c(lg_bauf_abortion_different:sgauo_womens_rights), ~replace_na(.x, 0)))
```
...
mod6b <- glmer(justicevot ~
                (1|id) + justicemq + justicefem + nonftyp2 + lcdisdirfem +
                lg_bauf_compulsory_motherhood_ii,
              family = binomial, data = data,  
              nAGQ = 0, control=glmerControl(
                check.conv.grad="ignore",check.conv.singular="ignore",check.conv.hess="ignore"))
