...
**Already done to the shortened-amicus csv file, skip this**
...
amicus <- amicus %>% 
  mutate(txt_full = str_to_lower(txt_short),
...
a <- amicus %>%
  mutate(txt_short = str_squish(txt_short),
    ...
    appendix_1a = str_count(txt_short, "\\*1a appendix|1a appendix|1 a appendix|\\*1a"),
...
```{r}
a %>% filter(brief== "Gonzales v Carhart. Amici Brief for Respondents (feminist), by Constitutional Law Professors et al.docx")
```


```{r}
a %>% filter(appendix_1a==0) %>% select(brief) %>% sample_n(1)
```
...
**We don't need to worry about the 'omitted footnote' language in the text. Remove text after 'footnotes counsel of record' or 'footnotes 1' (see github issue)**
