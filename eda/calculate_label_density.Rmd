...
# d1
d1_am <- labeled_amicus %>% filter(!is.na(divided_1)) %>% 
  select(case:fem, -text, divided_1,d1_label_code) %>% 
  rename(text = divided_1,label = d1_label_code)
```

```{r}
labeled_amicus %>% 
  filter(!is.na(divided_2) & !is.na(divided_1))
```
